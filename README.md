...
| **ForecastCard — Óránkénti oszlopdiagram** | kWh/óra | Open-Meteo `shortwave_radiation` + Render `GET /daily` `hourly.producedKwh` | `pvCapacityKwp * (radiation / 1000) * performanceRatio` per óra (előrejelzés) + inverter `dailyEnergyKwh` delta per óra (valós). **Zöld oszlop** = valós termelés (Kiosk `dailyEnergyKwh` delta, Render API), **sárga oszlop** = előrejelzett termelés (Open-Meteo). Y-tengely: kWh, X-tengely: óra (3 óránként címkézve). A `dailyEnergyKwh` a Kiosk `realKpi.dailyEnergy` hardver számlálójából származik, a power curve pontoknál integrálással (5-min × kW) kerül kiszámításra. |
