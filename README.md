...
| **PhasePowerChip** (L1/L2/L3) | Import/Export per fázis (W) | Számolt | `importLx = powerImportW × (V_lx × A_lx / Σ(V×A))`, `exportLx = powerExportW × (V_lx × A_lx / Σ(V×A))`. A P1 meter nem jelenti a per-fázis import/export irányt (mindig 0.000), csak az összesített értéket és a per-fázis V/A-t. V×A arányos elosztés stabil, nem tüskéz |
| **HousePhaseChip** (L1/L2/L3) | Ház fogyasztás per fázis (W) | Számolt | `houseW × (V_lx × A_lx / Σ(V×A))` — a Hub T-5 szinkronizált `realConsumptionW` értéke elosztva a fázisok látszólagos teljesítménye (V×A) arányában. Stabil, nem tüskéz — ha a bojler bekapcsol, L1 árama nő, L1 részesedése nő. Összeg mindig = `houseW` |
